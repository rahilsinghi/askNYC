...
import { Star, MapPin, ArrowUpRight, Info } from 'lucide-react';
...
export default function EvidenceCard({ idText, title, type, rating, buttons, className, delay = 0, onExplore }: EvidenceCardProps) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-start justify-between mb-5">
        <div className="space-y-1">
          ...
          <h3 className="text-lg font-bold text-white tracking-tight font-syne leading-tight uppercase italic">{title}</h3>
        </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] text-white/30 font-bold uppercase tracking-[0.2em]">{type}</span>
          </div>
          {onExplore && (
            <button
              onClick={onExplore}
